...
var configSchema = exports_external.object({
  ...
  porUrl: exports_external.string(),
...
var fetchRiskAnalysis = (geminiApiKey, totalReserveScaled, totalSupply) => (sendRequester, config) => {
...
var updateReserves = (runtime2, totalReserveScaled, totalSupply, riskScore) => {
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 totalMinted, uint256 totalReserve, uint256 riskScore"), [totalSupply, totalReserveScaled, riskScore]);
...
var getOffChainReserves = (runtime2) => {
  runtime2.log(`fetching por url ${runtime2.config.porUrl}`);
...
var getRiskScore = (runtime2, totalReserveScaled, totalSupply) => {
  const httpCapability = new ClientCapability2;
  const geminiApiKey = runtime2.getSecret({ id: "GEMINI_API_KEY" }).result();
  const riskAnalysis = httpCapability.sendRequest(runtime2, fetchRiskAnalysis(geminiApiKey.value, totalReserveScaled, totalSupply), ConsensusAggregationByFields({
    riskScore: median
  }))(runtime2.config).result();
  runtime2.log(`RiskScore ${riskAnalysis.riskScore}`);
...
var onCronTrigger = (runtime2, payload) => {
  ...
  const totalReserve = getOffChainReserves(runtime2);
  const totalSupply = getOnChainSupply(runtime2);
  const riskScore = getRiskScore(runtime2, totalReserve, totalSupply);
  const txnHash = updateReserves(runtime2, totalReserve, totalSupply, riskScore);
  runtime2.log("Finished CronTrigger");
